    ...
    public static class NegocioComumSAE
    {
			...
			where TModel : EntidadeDB
		{
			...
			foreach (TModel item in listItens)
			{
				...
				Int32 ideNOTIFficador = item.GetType().GetProperty("Ano") != null ?
				...
				String chavePropriedade = String.IsNullOrEmpty(propriedadeNome) ? propertyName : concatenarPropriedadeNome ?
					String.Concat(String.Concat(propertyName, propriedadeNome), ideNOTIFficador) : String.Concat(propriedadeNome, ideNOTIFficador);
				...
				String chave = String.Format(formatoChave, item is AnoContratual ? prefixoChave : prefixoPropriedade
					, item is AnoContratual ? String.Format(valorAnoChave, ideNOTIFficador.ToString()) : chavePropriedade);
		...
		public static void CarregarENOTIFtade<TModel>(this Dictionary<string, string> dicionarioSAE
			, TModel objeto
			, string prefixo = ""
			, Boolean concatenarPrefixo = false
			, String propriedadeNome = ""
		...
		public static void LimparSincronizarDicionario(this Dictionary<string, string> dicionarioSAE
			, String sistema
			, SeNOTIFdoSAE seNOTIFdo)
		{
			List<SAEParametros> saeParametros = new SAEParametros().Consultar(p => p.Direcao == (Int32)seNOTIFdo && p.Sistema == sistema);
